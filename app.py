...
# =========================================================================
        ...
        st.markdown("""
            <div style="background-color: #2E7D32; padding: 20px; border-radius: 10px; margin-bottom: 25px;">
                ...
                <p style="color: #E8F5E9; margin: 5px 0 0 0; font-size: 14px; opacity: 0.9;">
                    Módulo independente de empréstimos, controle de empréstimo de material e devoluções
                </p>
            </div>
        ...
        # ---------------------------------------------------------------------
        # SUB-ABA 1: ITENS DISPONÍVEIS (COM BOTAO DE EXCLUIR EM CADA LINHA)
        # ---------------------------------------------------------------------
        if sub_emp == "Itens Disponíveis":
            st.subheader("📋 Painel de Itens Cadastrados")
            ...
            if not itens_cadastrados:
                ...
                col_cod, col_nome, col_qtd, col_obs, col_acao = st.columns([1.5, 3, 2, 3, 1.5])
                col_cod.markdown("**Código**")
                col_nome.markdown("**Item / Equipamento**")
                col_qtd.markdown("**Total / Disp. / Emp.**")
                col_obs.markdown("**Observações**")
                col_acao.markdown("**Ação**")
                st.divider()

                # Lista cada item com seu respectivo botão de Excluir
                for i in itens_cadastrados:
                    c_id, c_cod, c_item, c_total, c_disp, c_obs = i
                    c_emp = c_total - c_disp
                    
                    c1, c2, c3, c4, c5 = st.columns([1.5, 3, 2, 3, 1.5])
                    c1.write(c_cod if c_cod else "-")
                    c2.write(f"**{c_item}**")
                    c3.write(f"{c_total} / {c_disp} / {c_emp}")
                    c4.write(c_obs if c_obs else "-")
                    
                    # Botão individual de exclusão
                    if c5.button("🗑️ Excluir", key=f"del_item_{c_id}"):
                        sucesso = excluir_item_emprestimo(c_id)
                        if sucesso:
                            st.success(f"Item '{c_item}' excluído com sucesso!")
                            st.rerun()
                        else:
                            st.error("Erro ao excluir item do banco de dados.")

        # ---------------------------------------------------------------------
        # SUB-ABA 2: CADASTRAR ITEM
        # ---------------------------------------------------------------------
                ...
                if st.form_submit_button("Cadastrar Item", type="primary"):
                    if item_emp.strip():
                        try:
                            ...
                            conn.commit()
                            st.success(f"Item '{item_emp}' cadastrado com sucesso!")
                            st.rerun()
                        except psycopg2.IntegrityError:
                            conn.rollback()
                            st.error("Erro: Já existe um item com este mesmo Código/Patrimônio.")
                    else:
                        st.error("O campo 'Nome do Item' é obrigatório!")

        # ---------------------------------------------------------------------
        # SUB-ABA 3: REGISTRAR SAÍDA
        # ---------------------------------------------------------------------
                ...
                with st.form("form_registro_devolucao", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Confirmar Devolução", type="primary"):
                        if pessoa_devolvendo.strip():
                            ...
                            st.error("Por favor, informe quem está efetuando a devolução.")

        # ---------------------------------------------------------------------
        # SUB-ABA 5: HISTÓRICO DE MOVIMENTAÇÃO
        # ---------------------------------------------------------------------
        elif sub_emp == "Histórico de Movimentação":
            st.subheader("Histórico de Empréstimos e Devoluções")
            ...
            if df_hist.empty:
                st.info("Nenhum histórico registrado no módulo de empréstimos.")
            else:
