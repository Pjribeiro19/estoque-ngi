...
def realizar_login(u, s):
    ...
    # Validação correta buscando a linha exata correspondente ao usuário digitado
    if u in df_user["Usuario"].values:
        usuario_linha = df_user[df_user["Usuario"] == u].iloc[0]
        senha_correta = str(usuario_linha["Senha"])
        
        if str(s) == senha_correta:
            st.session_state.logado = True
            st.session_state.usuario_atual = usuario_linha["Nome"]
...
# Interface Gráfica de Login (Força o bloqueio total se não estiver logado)
if not st.session_state.logado:
    ...
    st.stop() # Garante o bloqueio absoluto das telas de baixo
