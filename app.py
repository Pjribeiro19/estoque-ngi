...
# Inicializa o banco de dados de forma limpa
banco.inicializar_banco_automatico()
...
# Estilização CSS profissional atualizada
st.markdown("""
    <style>
    ...
    .img-container {
        ...
        width: 100%; margin-bottom: 20px; background-color: white; 
                    ...
                    if cursor.fetchone()[0] > 0:
                        if EMAIL_REMETENTE == "configurar_no_secrets@email.com":
                            ...
                            try:
                                ...
                                msg.attach(MIMEText("Sua senha provisória é: 123", 'plain'))
...
# --- SISTEMA PRINCIPAL ---
    ...
    with st.sidebar:
        ...
        # Implementação do menu profissional flat com ícones dinâmicos Lucide/FontAwesome
        escolha = option_menu(
            menu_title=None,
            options=[
                "Painel Geral",
            ...
            icons=[
                ...
                "arrow-left-right", # Movimentações
            ...
            styles={
                ...
                "nav-link": {
                    "font-size": "13px", "text-align": "left", "margin": "0px", "color": "var(--text-color)",
                    "--hover-color": "rgba(76, 175, 80, 0.15)"
                },
                "nav-link-selected": {"background-color": "rgba(76, 175, 80, 0.25)", "color": "#4CAF50", "font-weight": "bold"},
            }
        )
    ...
    if escolha == "Sair":
        ...
        with aba_gerenciar_prod:
            if not df_produtos.empty:
                ...
                edit_qtd = col_ed1.number_input("Quantidade:", min_value=0, value=int(df_raw_prod.loc[opcao_selecionada, "quantidade"]))
                ...
                edit_val = st.number_input("Valor Unitário:", value=float(df_raw_prod.loc[opcao_selecionada, "valor_unitario"]))
                
                c_prod1, c_prod2 = st.columns([1, 4])
                with c_prod1:
                    if st.button("Salvar Alterações", type="primary"):
                        cursor = conn.cursor()
                        cursor.execute("UPDATE produtos SET codigo=?, item=?, quantidade=?, categoria=?, valor_unitario=? WHERE codigo=?", (edit_cod.strip(), edit_item.strip(), edit_qtd, edit_cat, float(edit_val), cod_atual))
                        conn.commit()
            ...
            if not df_raw_users.empty:
                ...
                with c_btn_u1:
                    if st.button("Atualizar Dados", type="primary"):
                        cursor = conn.cursor()
                        cursor.execute("UPDATE usuarios SET nome=?, email=?, senha=?, perfil=? WHERE email=?", (edit_n.strip(), edit_e.strip().lower(), edit_s, edit_p, email_chave))
                        conn.commit()
                        st.rerun()
                with c_btn_u2:
                ...
                if st.form_submit_button("Cadastrar", type="primary"):
                    if s_coord and nc:
                        try:
                            ...
                            conn.commit()
                            st.rerun()
                        except: st.error("Sigla já existe.")
        with aba_c2:
        ...
        with aba_entrada:
            if df_raw_prod.empty: st.info("Sem materiais.")
            else:
                with st.form("form_registrar_entrada", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Confirmar Entrada", type="primary"):
                        ...
                        cursor.execute("INSERT INTO movimentacoes (data, tipo, codigo, item, quantidade, responsavel, coordenacao) VALUES (?, ?, ?, ?, ?, ?, ?)", (data_entrada.strftime("%d/%m/%Y"), "Entrada", cod_p, nome_p, qtd_entrada, "Almoxarifado", "-"))
                        conn.commit()
                        st.rerun()
            ...
            if df_raw_prod.empty: st.info("Sem materiais.")
                ...
                with st.form("form_registrar_saida", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Confirmar Saída", type="primary"):
                        ...
                        if not resp_retirada.strip(): st.error("Insira o responsável!")
                        elif qtd_saida > qtd_disp: st.error("Estoque insuficiente!")
                        else:
                            novo_saldo = qtd_disp - qtd_saida
                            cursor = conn.cursor()
                            cursor.execute("UPDATE produtos SET quantidade = ? WHERE codigo = ?", (novo_saldo, cod_p))
                            cursor.execute("INSERT INTO movimentacoes (data, tipo, codigo, item, quantidade, responsavel, coordenacao) VALUES (?, ?, ?, ?, ?, ?, ?)", (data_saida.strftime("%d/%m/%Y"), "Saída", cod_p, nome_p, qtd_saida, resp_retirada.strip(), coord_retirada))
                            conn.commit()
                            st.rerun()
        with aba_historico:
