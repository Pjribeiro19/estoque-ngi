...
def preservar_preenchimento(form_html, dados):
    ...
    (function () {
        ...
        ['calcularPernoites', 'atualizarBlocoAuxilio', 'atualizarQuantidadeAuxilio',
         ...
         'recalcularSeJaPreenchido', 'verificarTodasAsTarifas'].forEach(function (fn) {
...
def paginar(consulta, ordenacao, rota, por_pagina=50):
    ...
    anterior = (f'<a href="{url_for(rota, pagina=pagina - 1)}" class="btn-atalho">Anterior</a>'
                if pagina > 1 else '')
    proxima = (f'<a href="{url_for(rota, pagina=pagina + 1)}" class="btn-atalho">Próxima</a>'
...
def acao_analise(solicitacao_id):
    ...
    if acao == 'enviar':
        ...
        for aprovador in aprovadores:
            enviar_email(
                ...
                f'A solicitação {protocolo(solicitacao)} de {tipo_label} passou pela triagem e aguarda sua aprovação.',
            )
    ...
    if acao == 'reprovar':
        ...
        notificar_solicitante(
            ...
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} foi reprovada na análise.\n\n'
    ...
    if acao == 'devolver':
        ...
        notificar_solicitante(
            ...
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} foi devolvida para ajustes.\n\n'
...
def acao_aprovacao(solicitacao_id):
    ...
    if acao == 'aprovar':
        ...
        corpo_aprovacao = (
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} foi aprovada e seguiu para execução.'
        )
        ...
        if solicitacao.tipo == 'bolsa':
            ...
            enviar_email(
                ...
                f'A solicitação {protocolo(solicitacao)} de {tipo_label}, de {solicitacao.solicitante.nome}, '
                f'foi aprovada e está sob sua responsabilidade para encaminhamento.\n\n'
            ...
            for comprador in compradores:
                enviar_email(
                    ...
                    f'A solicitação {protocolo(solicitacao)} de {tipo_label} foi aprovada e está aguardando execução.',
                )
    ...
    if acao == 'reprovar':
        ...
        notificar_solicitante(
            solicitacao,
            'Sua solicitação foi reprovada - SIGAD Carajás',
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} foi reprovada na aprovação.\n\n'
            f'Justificativa: {justificativa}',
        )
    ...
    if acao == 'devolver':
        ...
        notificar_solicitante(
            solicitacao,
            'Sua solicitação precisa de ajustes - SIGAD Carajás',
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} foi devolvida para ajustes pelo Aprovador.\n\n'
            f'Motivo: {justificativa}',
        )
...
def fila_execucao():
    ...
    solicitacoes, navegacao = paginar(consulta, Solicitacao.data_envio.asc(), 'fila_execucao')
    ...
    for s in solicitacoes:
        ...
        <tr>
            ...
            <td>{responsavel}</td>
            <td>{prazo}</td>
            <td style="color:{cor}; font-weight:bold; font-size:12px;">{status_label}</td>
            <td>
                {moeda(s.valor_total)}
...
def acao_execucao(solicitacao_id):
    ...
    if acao == 'definir_prazo':
        ...
        notificar_solicitante(
            ...
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} está em execução, '
            f'com prazo de atendimento previsto para {solicitacao.prazo_encaminhamento.strftime("%d/%m/%Y")}.',
        )
    ...
    if acao == 'enviar_pagamento':
        ...
        notificar_solicitante(
            ...
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} foi cadastrada e enviada para pagamento.',
        )
    ...
    if acao == 'devolver_solicitante':
        ...
        notificar_solicitante(
            ...
            f'A solicitação {protocolo(solicitacao)} de {tipo_label} precisa de correção '
            f'antes de ser concluída.\n\n'
            f'O que corrigir: {motivo}\n\n'
            f'Acesse "Minhas Solicitações", corrija os dados e reenvie. '
            f'A solicitação volta direto para o responsável pelo encaminhamento, '
            f'sem precisar passar novamente pela análise e pela aprovação.',
        )
    ...
    if acao == 'em_compra':
        ...
        notificar_solicitante(
            ...
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} está em processo de compra.',
        )
    ...
    if acao == 'marcar_comprado':
        ...
        corpo_email = f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} foi comprada e está concluída.'
    ...
    if acao == 'marcar_paga':
        arquivo = request.files.get('comprovante')
        if not arquivo or not arquivo.filename:
            flash('Anexe o comprovante de pagamento para concluir a demanda.')
            return redirect(url_for('detalhe_solicitacao', solicitacao_id=solicitacao_id))
        ...
        notificar_solicitante(
            ...
            f'Sua solicitação {protocolo(solicitacao)} de {tipo_label} foi paga. '
            f'O comprovante de pagamento está disponível no sistema.',
        )

        flash('Demanda concluída e comprovante anexado.', 'sucesso')
        return redirect(url_for('detalhe_solicitacao', solicitacao_id=solicitacao_id))
...
def cadastro_usuarios_adicionar():
    ...
    if not senha or len(senha.strip()) < SENHA_MINIMA:
        flash(f'A senha inicial deve ter pelo menos {SENHA_MINIMA} caracteres.')
        return redirect(url_for('cadastro_usuarios'))
...
def cadastro_usuarios_atualizar(usuario_id):
    ...
    registrar_auditoria('alterou_usuario', None,
                        f'{usuario.nome}: perfil {perfil_anterior} -> {usuario.perfil}'
...
<form method="POST" enctype="multipart/form-data" style="max-width: 600px;" id="form-diaria">
    ...
    <div style="font-size:11px; color:#888; margin-bottom:10px;">
        Valor unitário multiplicado pelo número de pernoites. Viagem sem pernoite conta como uma diária.
    </div>
    ...
    <input type="text" name="cpf_diarista" id="cpf_diarista" required placeholder="000.000.000-00"
           maxlength="14" inputmode="numeric"
           style="padding:6px; margin-bottom:10px; width:190px;"><br>
    <div id="alerta_prestacao" style="display:none; background:#fdeceb; border-left:4px solid #c0392b;
...
function recalcularTotais() {
    ...
    var totalDiarias = valorUnitarioDiaria * quantidade;
...
var campoCpfDiarista = document.getElementById('cpf_diarista');
var alertaPrestacao = document.getElementById('alerta_prestacao');

campoCpfDiarista.addEventListener('blur', function() {
    var cpf = campoCpfDiarista.value.replace(/[^0-9]/g, '');
    if (cpf.length !== 11) {
        ...
        .then(function(dados) {
            ...
            if (dados.bloqueado) {
                alertaPrestacao.innerHTML =
                    '<strong>Envio bloqueado: este CPF possui prestação de contas não aprovada.</strong>' +
                    ...
                    'viagem anterior for <strong>aprovado</strong> pelo Executor ou pelo Analista.';
                alertaPrestacao.style.background = '#fdeceb';
                alertaPrestacao.style.borderLeftColor = '#c0392b';
                alertaPrestacao.style.color = '#a02020';
                window.__CPF_BLOQUEADO__ = true;
            } else {
                window.__CPF_BLOQUEADO__ = false;
            }
            alertaPrestacao.style.display = 'block';
        })
        .catch(function() { alertaPrestacao.style.display = 'none'; window.__CPF_BLOQUEADO__ = false; });
});

document.getElementById('form-diaria').addEventListener('submit', function (evento) {
    if (window.__CPF_BLOQUEADO__) {
        evento.preventDefault();
        alert('Não é possível enviar: o CPF informado tem prestação de contas de diária anterior ainda não aprovada.');
    }
});
...
def diaria_form():
    if request.method == 'POST':
        ...
        if not cpf_tem_11_digitos(cpf_diarista):
            flash('Informe o CPF do diarista com 11 dígitos, no formato 000.000.000-00.')
            return render_pagina('Solicitação de Diária',
                                 preservar_preenchimento(montar_formulario_diaria(), request.form))

        bloqueantes = prestacoes_nao_aprovadas_por_cpf(cpf_diarista)
        if bloqueantes:
            protocolos = ', '.join(protocolo(sol) for sol, dia in bloqueantes)
            flash(f'Não é possível enviar: este CPF tem prestação de contas de diária anterior '
                  f'ainda não aprovada ({protocolos}). Aguarde a aprovação do relatório de viagem '
                  f'antes de solicitar uma nova diária para esta pessoa.')
            return render_pagina('Solicitação de Diária',
                                 preservar_preenchimento(montar_formulario_diaria(), request.form))

        telefone_ok, telefone_diarista = montar_telefone(
            request.form.get('ddd_diarista'), request.form.get('telefone_diarista'))
        if not telefone_ok:
            flash('Informe o telefone do diarista: DDD com 2 dígitos e número com 8 ou 9 dígitos.')
            return render_pagina('Solicitação de Diária',
                                 preservar_preenchimento(montar_formulario_diaria(), request.form))
        ...
        valor_total_solicitacao = (valor_diaria * quantidade_diarias) + valor_auxilio_total
        ...
        diaria = SolicitacaoDiaria(
            ...
            cpf_diarista=formatar_cpf(cpf_diarista),
            telefone_diarista=telefone_diarista,
    ...
    return render_pagina('Solicitação de Diária', montar_formulario_diaria())
...
def passagem_form():
    ...
    return render_pagina('Solicitação de Passagem', montar_formulario_passagem())
...
def compra_materiais_form():
    ...
    return render_pagina('Solicitação de Compra de Materiais', montar_formulario_compras())
...
function htmlPrestadorPF(numero) {
    ...
    '<div class="bloco-prestador bloco" style="margin-bottom:15px;">' +
        ...
        '<input type="text" class="campo-valor-display" required style="padding:6px; margin-bottom:10px; width:130px;" value="0,00">' +
...
function ativarPrestador(bloco) {
    ...
    display.addEventListener('input', function () {
        if (!this.readOnly) {
            ...
            if (digitos === '') { oculto.value = '0'; atualizarTotalPF(); return; }
            var reais = parseInt(digitos, 10) / 100;
            oculto.value = reais.toFixed(2);
...
def servico_externo_pf_form():
    if request.method == 'POST':
        ...
        for i in range(len(nomes)):
            ...
            valor = float(valores[i] or 0) if i < len(valores) else 0
            valor_total_solicitacao += valor
    ...
    return render_pagina('Serviço Externo - Pessoa Física', montar_formulario_pf())
...
def servico_externo_pj_form():
    ...
    return render_pagina('Serviço Externo - Pessoa Jurídica', montar_formulario_pj())
...
// A tabela de referência (POF/IBGE) considera as três refeições combinadas.
// Fornecer só uma refeição reduz a quantidade proporcionalmente - a
// proporção é uma estimativa (café ~25%, almoço ~40%, jantar ~35% do
// consumo diário), não um dado quebrado por refeição na pesquisa original.
var MULTIPLICADOR_REFEICAO = { todas: 1, cafe: 0.25, almoco: 0.40, jantar: 0.35 };

function obterMultiplicadorRefeicao() {
    var selecao = document.getElementById('tipo_refeicao').value;
    return MULTIPLICADOR_REFEICAO[selecao] || 1;
}
...
function calcularQuantidades() {
    ...
    var multiplicador = obterMultiplicadorRefeicao();
    ...
    document.querySelectorAll('.qtd-item').forEach(function(campo) {
        ...
        if (fator > 0) {
            var sugerido = Math.ceil(fator * pessoas * dias * multiplicador);
...
def rancho_form():
    if request.method == 'POST':
        ...
        MULTIPLICADOR_REFEICAO = {'todas': 1, 'cafe': 0.25, 'almoco': 0.40, 'jantar': 0.35}
        tipo_refeicao = request.form.get('tipo_refeicao') or 'todas'
        multiplicador_refeicao = MULTIPLICADOR_REFEICAO.get(tipo_refeicao, 1)
        ...
        for i in range(len(ids_itens)):
            ...
            # mesma regra do navegador: quantidade acima da sugestão calculada
            # pelo fator de consumo (POF/IBGE, ajustado pelas refeições
            # selecionadas) exige justificativa
            fator = float(item.fator_consumo or 0)
            if fator > 0 and num_pessoas > 0 and num_dias > 0:
                sugerido = math.ceil(fator * num_pessoas * num_dias * multiplicador_refeicao)
    ...
    return render_pagina('Solicitação de Rancho', montar_formulario_rancho())
...
def cadastro_rancho():
    ...
    for item in itens:
        ...
        <tr>
            ...
            <td><input type="text" name="nome" value="{item.nome}" style="width:220px; padding:4px;"></td>
...
<form method="POST" style="max-width: 750px;" id="form-seguro">
    <h3>Dados gerais</h3>
    <label>Ponto Focal da solicitação: <span style="color:red;">*</span></label><br>
    <input type="text" name="ponto_focal" required style="width:100%; padding:6px; margin-bottom:10px;"><br>
...
__CAMPO_CONVENIO_SEGURO__
    ...
    <input type="text" id="quantidade_pessoas_display" readonly style="padding:6px; margin-bottom:10px; background:#f5f5f5; width:100px;" value="0">
    <input type="hidden" name="quantidade_pessoas" id="quantidade_pessoas_hidden" value="0">
    <div style="font-size:11px; color:#888; margin-bottom:10px;">
        Preenchido automaticamente conforme os participantes adicionados na lista abaixo.
    </div>
    ...
    <div style="font-size:11px; color:#888; margin-bottom:10px;">
        Todos os campos de cada participante são obrigatórios. Adicione um bloco para cada pessoa a ser segurada.
    </div>
...
<template id="template-participante">
    <div class="bloco-participante bloco" style="margin-bottom:15px;">
        <strong>Participante <span class="numero-participante"></span></strong>
...
function renumerarParticipantes() {
    var blocos = document.querySelectorAll('.bloco-participante');
    blocos.forEach(function(bloco, indice) {
        bloco.querySelector('.numero-participante').textContent = indice + 1;
    });
    document.getElementById('quantidade_pessoas_display').value = blocos.length;
    document.getElementById('quantidade_pessoas_hidden').value = blocos.length;
}
...
function criarBlocoParticipante() {
    ...
    clone.querySelector('.btn-remover-participante').addEventListener('click', function() {
        bloco.remove();
...
if (!window.__RESTAURAR_FORM__) { criarBlocoParticipante(); }
verificarKm();
</script>
"""
...
def seguro_form():
    if request.method == 'POST':
        ...
        for i in range(len(nomes)):
            ...
            if not nome:
                continue
            cpf_participante = cpfs[i] if i < len(cpfs) else ''
            ...
            telefone_ok, telefone_participante = montar_telefone(
                ddds[i] if i < len(ddds) else '',
                telefones[i] if i < len(telefones) else '')
            if not telefone_ok:
            ...
            participantes.append({
                'nome_completo': nome,
                'data_nascimento': nascimentos[i] if i < len(nascimentos) else None,
                ...
                'uf': ufs[i] if i < len(ufs) else '',
                'cep': ceps[i] if i < len(ceps) else '',
                'ddd': ddds[i] if i < len(ddds) else '',
...
"""
    ...
    form_html = SEGURO_FORM_TEMPLATE.replace('__OPCOES_COORDENACAO__', montar_opcoes_coordenacoes())
    form_html = form_html.replace('__CAMPO_CONVENIO_SEGURO__', campo_convenio)
    form_html = form_html.replace('__OPCOES_ESTADOS__', montar_opcoes_estados())
    return render_pagina('Solicitação de Seguro', form_html)


# ---------------- SOLICITAÇÃO: BOLSA ----------------
BOLSA_FORM_TEMPLATE = """
<div style="margin-bottom:14px;"><a href="/ajuda#bolsa" target="_blank" class="btn-atalho">Dúvidas sobre bolsas? Consulte a Central de Ajuda</a></div>
<form method="POST" enctype="multipart/form-data" style="max-width: 800px;" id="form-bolsa">
    <h3>Dados gerais</h3>
    <label>Ponto Focal da solicitação: <span style="color:red;">*</span></label><br>
    <input type="text" name="ponto_focal" required style="width:100%; padding:6px; margin-bottom:10px;"><br>
...
__CAMPOS_ANALISTA__
    ...
    <h3>Bolsistas</h3>
    <div style="font-size:11px; color:#888; margin-bottom:10px;">
        A duração em meses e o valor total de cada bolsa são calculados automaticamente
        a partir do mês de início, do mês de fim e do valor mensal.
    </div>
    <div id="bolsistas-container"></div>

    <button type="button" id="btn-adicionar-bolsista" class="btn-atalho" style="margin-top:10px;">+ Adicionar bolsista</button>

    <h3 style="margin-top:20px;">Valor total da solicitação</h3>
    <input type="text" id="valor_total_display" readonly style="padding:8px; background:#f5f5f5; width:200px; font-weight:bold; font-size:15px;" value="R$ 0,00">
    <input type="hidden" name="valor_total_calculado" id="valor_total_hidden" value="0"><br><br>

    <label>Anexar Plano de Trabalho (opcional):</label><br>
    <input type="file" name="anexos_plano" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" style="margin-bottom:15px;"><br>
...
<template id="template-bolsista">
    <div class="bloco-bolsista bloco" style="margin-bottom:15px;">
        <strong>Item <span class="numero-bolsista"></span></strong>
        <button type="button" class="btn btn-excluir btn-remover-bolsista" style="float:right; padding:4px 10px;">Remover</button>
        <div style="clear:both;"></div>

        <label>Nome completo do bolsista: <span style="color:red;">*</span></label><br>
        ...
        <label>Projeto relacionado: <span style="color:red;">*</span></label><br>
        <input type="text" name="bolsa_projeto[]" required style="width:100%; padding:6px; margin-bottom:10px;"><br>

        <label>Tipo de Bolsa: <span style="color:red;">*</span></label><br>
        <input type="text" name="bolsa_tipo[]" required placeholder="Ex: Iniciação científica, Pesquisador, Técnico" style="width:100%; padding:6px; margin-bottom:10px;"><br>

        <label>Mês de início da implementação: <span style="color:red;">*</span></label><br>
        ...
        <input type="month" name="bolsa_mes_fim[]" class="campo-mes-fim" required style="padding:6px; margin-bottom:10px;"><br>

        <label>Duração em meses (calculada automaticamente):</label><br>
        <input type="text" class="campo-duracao-display" readonly style="padding:6px; margin-bottom:10px; background:#f5f5f5; width:100px;" value="0">
        <input type="hidden" name="bolsa_duracao[]" class="campo-duracao-hidden" value="0"><br>
        <div class="aviso-periodo" style="font-size:11px; color:#a00; margin-bottom:10px; display:none;">
            O mês de fim deve ser igual ou posterior ao mês de início.
        </div>

        <label>Valor mensal da Bolsa (R$): <span style="color:red;">*</span></label><br>
        <input type="number" step="0.01" min="0" name="bolsa_valor_mensal[]" class="campo-valor-mensal" required style="padding:6px; margin-bottom:10px; width:150px;"><br>

        <label>Valor total da bolsa (R$):</label><br>
        <input type="text" class="campo-valor-total-display" readonly style="padding:6px; margin-bottom:10px; background:#f5f5f5; width:180px; font-weight:bold;" value="R$ 0,00">
        <input type="hidden" name="bolsa_valor_total[]" class="campo-valor-total-hidden" value="0"><br>

        <label>O bolsista precisará de crachá institucional? <span style="color:red;">*</span></label><br>
...
<script>
function formatarRealBolsa(valor) {
    return 'R$ ' + valor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

function calcularMeses(mesInicio, mesFim) {
    if (!mesInicio || !mesFim) {
        return 0;
    }
    var partesInicio = mesInicio.split('-');
    var partesFim = mesFim.split('-');
    var anoInicio = parseInt(partesInicio[0]);
    var mInicio = parseInt(partesInicio[1]);
    var anoFim = parseInt(partesFim[0]);
    var mFim = parseInt(partesFim[1]);

    var meses = (anoFim - anoInicio) * 12 + (mFim - mInicio) + 1;
    return meses;
}

function atualizarBloco(bloco) {
    var mesInicio = bloco.querySelector('.campo-mes-inicio').value;
    var mesFim = bloco.querySelector('.campo-mes-fim').value;
    var valorMensal = parseFloat(bloco.querySelector('.campo-valor-mensal').value) || 0;
    var aviso = bloco.querySelector('.aviso-periodo');

    var meses = calcularMeses(mesInicio, mesFim);

    if (mesInicio && mesFim && meses < 1) {
        aviso.style.display = 'block';
        meses = 0;
    } else {
        aviso.style.display = 'none';
    ...
    var total = meses * valorMensal;

    bloco.querySelector('.campo-duracao-display').value = meses;
    bloco.querySelector('.campo-duracao-hidden').value = meses;
    bloco.querySelector('.campo-valor-total-display').value = formatarRealBolsa(total);
    bloco.querySelector('.campo-valor-total-hidden').value = total.toFixed(2);
...
function atualizarTotalGeral() {
    var total = 0;
    document.querySelectorAll('.campo-valor-total-hidden').forEach(function(campo) {
        total += parseFloat(campo.value) || 0;
    });
    document.getElementById('valor_total_display').value = formatarRealBolsa(total);
    document.getElementById('valor_total_hidden').value = total.toFixed(2);
}
...
function criarBlocoBolsista() {
    ...
    ['.campo-mes-inicio', '.campo-mes-fim', '.campo-valor-mensal'].forEach(function(seletor) {
        bloco.querySelector(seletor).addEventListener('input', function() {
            atualizarBloco(bloco);
        });
    });

    clone.querySelector('.btn-remover-bolsista').addEventListener('click', function() {
        bloco.remove();
        renumerarBolsistas();
        atualizarTotalGeral();
    });
...
if (!window.__RESTAURAR_FORM__) { criarBlocoBolsista(); }
</script>
"""
...
def bolsa_form():
    if request.method == 'POST':
        ...
        if pode_definir_convenio():
            solicitacao.convenio = request.form.get('convenio')
            solicitacao.lote_aprovacao = request.form.get('lote_aprovacao')
            solicitacao.rubrica = request.form.get('rubrica')
    ...
    return render_pagina('Solicitação de Bolsa', montar_formulario_bolsa())
...
CAMPOS_EDICAO = {
    'diaria': (SolicitacaoDiaria, [
        ('nome_diarista', 'Nome do diarista', 'texto', True),
        ('data_ida', 'Data de ida', 'data', True),
        ('data_retorno', 'Data de retorno', 'data', True),
        ('cidade_origem', 'Cidade de origem', 'texto', True),
        ('estado_origem', 'UF de origem', 'uf', True),
        ('cidade_destino', 'Cidade de destino', 'texto', True),
        ('estado_destino', 'UF de destino', 'uf', True),
        ('tipo_destino', 'Tipo de diária (área)', 'area_diaria', True),
        ('tipo_diaria', 'Diária cheia ou meia', 'tipo_diaria', True),
        ('numero_pernoites', 'Nº de pernoites', 'numero', False),
        ('justificativa', 'Justificativa', 'area', True),
        ('cpf_diarista', 'CPF', 'texto', False),
        ('telefone_diarista', 'Telefone', 'texto', False),
        ('email_diarista', 'E-mail', 'texto', False),
        ('banco_diarista', 'Banco', 'texto', True),
        ('agencia_diarista', 'Agência', 'texto', True),
        ('conta_diarista', 'Conta', 'texto', True),
        ('chave_pix', 'Chave PIX', 'texto', True),
    ]),
    'alimentacao': (SolicitacaoAlimentacao, [
...
def corrigir_solicitacao(solicitacao_id):
    ...
    if request.method == 'POST':
        ...
        if retorno_ao_executor:
            ...
            if solicitacao.responsavel_encaminhamento:
                enviar_email(
                    ...
                    f'A solicitação {protocolo(solicitacao)} de {tipo_label}, de '
                    f'{solicitacao.solicitante.nome}, foi corrigida pelo solicitante e '
                    f'está de volta na sua fila para conclusão.',
                )
        ...
        for analista in Usuario.query.filter_by(perfil='analista').all():
            enviar_email(
                ...
                f'A solicitação {protocolo(solicitacao)} de {tipo_label}, de '
                f'{solicitacao.solicitante.nome}, foi corrigida e reenviada para análise.',
            )
    ...
    aviso_itens = ''
    if solicitacao.tipo in ('rancho', 'servico_externo', 'servico_externo_pf', 'servico_externo_pj',
                            'bolsa', 'seguro', 'compra_materiais', 'passagem'):
        aviso_itens = """
        <div class="bloco" style="background:#fff8e6; border-left:4px solid #b35c00;">
            <div style="font-size:12.5px; color:#7a4a00;">
                Esta solicitação possui uma lista de itens/participantes que não pode ser alterada nesta tela.
                Se a correção envolver a lista, cancele esta solicitação com o Analista e envie uma nova.
            </div>
        </div>
        """
...
def detalhe_solicitacao(solicitacao_id):
    ...
    if solicitacao.tipo == 'diaria':
        d = SolicitacaoDiaria.query.filter_by(solicitacao_id=solicitacao.id).first()
        if d:
            corpo += tabela('Dados da diária', [
                ('Nome do diarista', d.nome_diarista),
                ...
                ('Diária cheia/meia', d.tipo_diaria),
                ('Nº de pernoites', d.numero_pernoites),
                ('Valor da diária', dinheiro(d.valor_diaria)),
                ('Auxílio deslocamento', 'Sim' if d.tera_auxilio_deslocamento else 'Não'),
                ('Qtd. de auxílio', d.quantidade_auxilio),
                ('Valor do auxílio', dinheiro(d.valor_auxilio)),
                ('Justificativa do auxílio', d.justificativa_auxilio),
                ('Justificativa', d.justificativa),
            ])
            corpo += tabela('Dados bancários do diarista', [
                ('CPF', d.cpf_diarista), ('Telefone', d.telefone_diarista), ('E-mail', d.email_diarista),
                ('Banco', d.banco_diarista), ('Agência', d.agencia_diarista),
                ('Conta', d.conta_diarista), ('Chave PIX', d.chave_pix),
            ])
    ...
    if solicitacao.tipo == 'rancho':
        ...
        if rancho:
            ...
            <table style="max-width:700px; margin-bottom:20px;">
                ...
                <tr><th>Justificativa do aumento de quantidade</th><td>{rancho.justificativa_aumento or '-'}</td></tr>
        ...
        for item in itens:
            if item.categoria != categoria_atual:
                categoria_atual = item.categoria
                linhas_itens += f'<tr><td colspan="5" style="background:#e8eef3; font-weight:bold;">{categoria_atual}</td></tr>'
            ...
            <tr>
                ...
                <td style="text-align:center;">{float(item.quantidade or 0):.0f}</td>
        ...
        if not linhas_itens:
            linhas_itens = '<tr><td colspan="5">Nenhum item nesta solicitação.</td></tr>'
        ...
        <table style="max-width:750px;">
            <tr>
                <th>Item</th><th style="width:70px;">Unid.</th><th style="width:80px;">Qtd</th>
    ...
    if solicitacao.tipo == 'seguro':
        ...
        if not linhas_part:
            linhas_part = '<tr><td colspan="6">Nenhum participante cadastrado.</td></tr>'

        corpo += f"""
        <h3>Participantes do seguro de vida</h3>
        <table style="max-width:1000px;">
            <tr>
                <th>Nome completo</th><th>Nascimento</th><th>CPF</th>
    ...
    if anexos:
        ...
        for a in anexos:
            ...
            linhas_anexos += f"""
            <tr>
                <td style="font-size:12px;">{a.nome_arquivo}
                    {'<br><span style="color:#2e7d32; font-size:11px; font-weight:bold;">Comprovante de pagamento</span>' if a.tipo_anexo == 'comprovante_pagamento' else ('<br><span style="color:#2e7d32; font-size:11px; font-weight:bold;">Nota fiscal / comprovante da compra</span>' if a.tipo_anexo == 'comprovante_compra' else ('<br><span style="color:#37784D; font-size:11px; font-weight:bold;">Relatório de prestação de contas</span>' if a.tipo_anexo == 'prestacao_contas' else ''))}
                </td>
                <td style="font-size:11px; color:#666;">{a.data_upload.strftime('%d/%m/%Y %H:%M')}</td>
...
{campo_encaminhamento}
    ...
    if solicitacao.status in STATUS_EM_ANDAMENTO and pode_executar(solicitacao):
        ...
        if solicitacao.status == 'aprovada':
            ...
            bloco_acao = """
            <label>Comprovante de pagamento: <span style="color:red;">*</span></label><br>
            <input type="file" name="comprovante" accept=".pdf,.jpg,.jpeg,.png" style="margin-bottom:12px;"><br>
...
def editar_quantidade_alimentacao(solicitacao_id):
    ...
    notificar_solicitante(
        ...
        f'A quantidade de pessoas da sua solicitação {protocolo(solicitacao)} foi ajustada '
        f'de {quantidade_anterior} para {nova_quantidade}.\n\n'
...
def editar_item_rancho(solicitacao_id):
    ...
    notificar_solicitante(
        ...
        f'A quantidade do item "{item_solicitacao.nome_item}" na sua solicitação '
        f'{protocolo(solicitacao)} foi {sentido} de {quantidade_anterior:g} para '
...
def prestacoes_nao_aprovadas_por_cpf(cpf):
    """Diárias já retornadas (viagem concluída) cujo relatório ainda não foi
    aprovado. Usado para BLOQUEAR uma nova diária para o mesmo CPF - regra:
    o diarista só pode viajar de novo depois que a prestação anterior for
    aprovada, não bastando apenas entregá-la."""
    cpf_limpo = so_digitos(cpf)
    if len(cpf_limpo) != 11:
        return []
    ...
    registros = db.session.query(Solicitacao, SolicitacaoDiaria).join(
        ...
        Solicitacao.tipo == 'diaria',
        Solicitacao.status.in_(('aprovada', 'em_execucao', 'enviado_pagamento', 'paga')),
        Solicitacao.prestacao_aprovada_por.is_(None),
...
def registrar_prestacao_contas(solicitacao_id):
    ...
    diaria = SolicitacaoDiaria.query.filter_by(solicitacao_id=solicitacao.id).first()
    nome_diarista = diaria.nome_diarista if diaria else solicitacao.solicitante.nome
...
def avaliar_prestacao_contas(solicitacao_id):
    ...
    if acao == 'aprovar':
        ...
        notificar_solicitante(
            ...
            f'A prestação de contas da solicitação {protocolo(solicitacao)} foi conferida e '
            f'APROVADA por {current_user.nome}.\n\n'
    ...
    if acao == 'recusar':
        ...
        notificar_solicitante(
            ...
            f'O relatório de viagem da solicitação {protocolo(solicitacao)} foi conferido e '
            f'DEVOLVIDO para correção.\n\n'
            f'Motivo: {motivo}\n\n'
            f'Acesse o sistema, corrija as informações e envie o relatório novamente. '
            f'A pendência de prestação de contas permanece em aberto até a aprovação.',
        )
...
ITENS_RANCHO_PADRAO = [
    # (nome, categoria, unidade, fator por pessoa/dia, valor unitário de referência)
    ('Arroz (kg)', 'Mantimentos', 'kg', 0.2, 6.00),
    ('Feijão (kg)', 'Mantimentos', 'kg', 0.08, 9.50),
    ('Farinha de mandioca amarela (kg)', 'Mantimentos', 'kg', 0.05, 7.00),
    ('Macarrão (kg)', 'Mantimentos', 'kg', 0.06, 6.50),
    ('Molho de tomate (kg)', 'Mantimentos', 'kg', 0.03, 8.00),
    ('Açúcar (kg)', 'Mantimentos', 'kg', 0.05, 4.50),
    ('Cuzcuz (kg)', 'Mantimentos', 'kg', 0.05, 6.00),
    ('Tapioca pronta (kg)', 'Mantimentos', 'kg', 0.04, 9.00),
    ('Pão de forma (kg)', 'Mantimentos', 'kg', 0.05, 12.00),
    ('Margarina (kg)', 'Mantimentos', 'kg', 0.02, 12.00),
    ('Café (kg)', 'Mantimentos', 'kg', 0.02, 45.00),
    ('Leite em pó (kg)', 'Mantimentos', 'kg', 0.03, 32.00),
    ('Biscoito de água e sal (kg)', 'Mantimentos', 'kg', 0.03, 14.00),
    ('Biscoito doce (maisena) (kg)', 'Mantimentos', 'kg', 0.03, 14.00),
    ('Farinha de trigo (kg)', 'Mantimentos', 'kg', 0.03, 5.00),
    ('Fermento Químico (kg)', 'Mantimentos', 'kg', 0.005, 30.00),
    ('Goiabada (kg)', 'Mantimentos', 'kg', 0.02, 14.00),

    ('Sal (kg)', 'Temperos/condimentos', 'kg', 0.01, 3.00),
    ('Pimenta de reino (kg)', 'Temperos/condimentos', 'kg', 0.002, 80.00),
    ('Açafrão (kg)', 'Temperos/condimentos', 'kg', 0.002, 40.00),
    ('Coloral (kg)', 'Temperos/condimentos', 'kg', 0.002, 20.00),
    ('Óleo (litro)', 'Temperos/condimentos', 'litro', 0.03, 8.00),

    ('Alho (kg)', 'Frutas e legumes', 'kg', 0.005, 30.00),
    ('Batata Inglesa (kg)', 'Frutas e legumes', 'kg', 0.05, 7.00),
    ('Cebola (kg)', 'Frutas e legumes', 'kg', 0.03, 6.00),
    ('Cenoura (kg)', 'Frutas e legumes', 'kg', 0.03, 6.00),
    ('Tomate (kg)', 'Frutas e legumes', 'kg', 0.05, 10.00),
    ('Repolho (kg)', 'Frutas e legumes', 'kg', 0.03, 5.00),
    ('Cheiro Verde (kg)', 'Frutas e legumes', 'kg', 0.01, 20.00),
    ('Banana (kg)', 'Frutas e legumes', 'kg', 0.1, 6.00),
    ('Maçã (kg)', 'Frutas e legumes', 'kg', 0.08, 12.00),
    ('Melancia (kg)', 'Frutas e legumes', 'kg', 0.15, 4.00),

    ('Frango Inteiro (un)', 'Proteínas', 'un', 0.05, 20.00),
    ('Calabresa (kg)', 'Proteínas', 'kg', 0.03, 25.00),
    ('Sardinha (kg)', 'Proteínas', 'kg', 0.03, 30.00),
    ('Ovo (un)', 'Proteínas', 'un', 0.5, 1.00),
    ('Carne vermelha (kg)', 'Proteínas', 'kg', 0.15, 38.00),

    ('Suco em pó (pacote de 1 kg)', 'Bebidas', 'pacote', 0.02, 18.00),

    ('Papel Higiênico (rolos)', 'Higiene e Limpeza', 'rolos', 0.15, 1.50),
    ('Água sanitária 1 L', 'Higiene e Limpeza', 'un', 0, 5.00),
    ('Desinfetante 1L', 'Higiene e Limpeza', 'un', 0, 6.00),
    ('Detergente 500 ml', 'Higiene e Limpeza', 'un', 0, 3.00),
    ('Sabão em pó 1Kg', 'Higiene e Limpeza', 'un', 0, 12.00),
    ('Sabão em barra 1Kg', 'Higiene e Limpeza', 'un', 0, 8.00),
    ('Saco de lixo 50 L', 'Higiene e Limpeza', 'pacote', 0, 15.00),
    ('Botijão de Gás', 'Higiene e Limpeza', 'un', 0, 120.00),
]


def seed_itens_rancho():
    for ordem, (nome, categoria, unidade, fator, valor) in enumerate(ITENS_RANCHO_PADRAO, start=1):
        if not ItemRancho.query.filter_by(nome=nome).first():
            db.session.add(ItemRancho(
                nome=nome,
                categoria=categoria,
                unidade=unidade,
