...
def montar_checkboxes_itens_aprovacao(solicitacao):
    """Monta a lista de itens desta solicitação, cada um com checkbox para
    o Aprovador reprovar individualmente (com justificativa). Usado dentro
    do formulário de decisão do Aprovador. Devolve string vazia para tipos
    de item único."""
    config = MODELO_ITEM_POR_TIPO.get(solicitacao.tipo)
    if not config:
        return ''

    modelo, _campo_valor, campo_nome = config
    itens = modelo.query.filter_by(solicitacao_id=solicitacao.id).all()
    if not itens:
        return ''
    ...
    return f"""
    <div class="bloco" style="border-left:4px solid #b35c00; max-width:750px; margin-bottom:14px;">
        <strong style="font-size:13px;">Aprovação item a item (opcional)</strong>
        <div style="font-size:11px; color:#888; margin:4px 0 10px;">
            Por padrão, todos os itens são aprovados junto com a solicitação. Marque só os
            itens que NÃO devem ser aprovados - o restante segue normalmente para execução.
        </div>
        {linhas}
    </div>
    <script>
    document.querySelectorAll('.checkbox-reprovar-item').forEach(function (caixa) {{
        caixa.addEventListener('change', function () {{
            ...
            bloco.style.display = caixa.checked ? 'block' : 'none';
        }});
    }});
    </script>
    """
...
{campo_encaminhamento}
        ...
        checkboxes_itens = montar_checkboxes_itens_aprovacao(solicitacao)
